...
const DEFAULT_ENGLISH_TO_CHINESE_PROMPT: &str = "Translate the input into natural, accurate Mainland China Simplified Chinese (简体中文). You MUST output only Simplified Chinese characters and MUST NOT use any Traditional Chinese characters (繁體字) under any circumstances. Preserve meaning, tone, and formatting when possible. If the input is a table, output a valid GitHub-flavored Markdown table with a proper |---| delimiter row; otherwise output plain text. Never output a partially-formatted or half-rendered table. Return only the final Simplified Chinese translation with no explanation.";
